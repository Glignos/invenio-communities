...
from invenio_communities.records.api import CommunityRecord, \
    CommunityInclusionRequest
from invenio_communities.members.api import CommunityMembersAPI
from invenio_communities.utils import record_resolver
...
@ui_blueprint.route('/communities/<{0}:pid_value>/curation'.format(
...
class ListResource(MethodView):
    ...
    def post(self, pid=None, community=None, record_pid=None,
        ...
        record_pid, record = record_resolver.resolve(record_pid)
        try:
...
api_blueprint.add_url_rule(
    '/communities/<{0}:pid_value>/requests/inclusion/<request_id>'.format(
        'pid(comid,record_class="invenio_communities.api:Community",'
        'object_type="com")'),
...
api_blueprint.add_url_rule(
    '/communities/<{0}:pid_value>/requests/inclusion/<request_id>/<any(accept,reject,comment):action>'.format(
        'pid(comid,record_class="invenio_communities.api:Community",'
        'object_type="com")'),
